...
PATCH_MPQ = os.path.join(PROJECT_ROOT, "input", "patch", "patch-B.mpq")
MERGED_DBC = os.path.join(PROJECT_ROOT, "output", "extract", "merged", "DBFilesClient", "Spell.dbc")
...
if not os.path.exists(PATCH_MPQ) and os.path.exists(root_patch):
    os.makedirs(os.path.dirname(PATCH_MPQ), exist_ok=True)
    shutil.copy(root_patch, PATCH_MPQ)

if not os.path.exists(PATCH_MPQ):
    print(f"ERROR: Patch MPQ not found: {PATCH_MPQ}")
    exit(1)
